...
public class FlyingDroneScript : MonoBehaviour {
	...
	private void UpdateFreeMovementState()
	{
		...
		try {
			float dr  = CnInputManager.GetAxis ("DroneRotate");
 			float dfb = CnInputManager.GetAxis ("DroneForwardBack");
			float dud = CnInputManager.GetAxis ("DroneUpDown");
			float drl = CnInputManager.GetAxis ("DroneRightLeft");
            // Adjustable force.
	...
	void Start () {
        ...
        GetComponent<Rigidbody>().centerOfMass = new Vector3(0, -2, CenterOfMass); //WC(0, -2, -15);
