...
namespace SmartTrackSystem
{
    ...
    {
        [SerializeField, HideInInspector] private List<GameObject> gameObjectsToRecord = new List<GameObject>() { };
        ...
        public class RovItens 
        {
            ...
            public List<RigmasterTotal> rigMasterTotals = new List<RigmasterTotal>();
            public List<RigMasterControl> rigMasterControls = new List<RigMasterControl>();
            public List<T4FORCE2> t4FORCE2s = new List<T4FORCE2>();
            public List<RigControlNew> rigControlNews = new List<RigControlNew>();
            public List<GenericTranslate> genericTranslates = new List<GenericTranslate>();
        ...
        [SerializeField, HideInInspector] private List<Matrix> ropesInitialIndexes = new List<Matrix>();
        ...
        private void HideConsole() { console.gameObject.SetActive(!console.gameObject.activeSelf);}
        ...
        {
            yield return FileBrowser.WaitForSaveDialog(FileBrowser.PickMode.Files, false, null, null, "Save Files", "Save");
        ...
        private void PrepareToReplay()
        {
            ...
            foreach (RigmasterTotal rmt in rov.GetComponentsInChildren<RigmasterTotal>()) {
                if (rmt.enabled){
                    rovItens.rigMasterTotals.Add(rmt);
                    rmt.enabled = false;
                }
            }

            foreach (RigMasterControl rmc in rov.GetComponentsInChildren<RigMasterControl>()) {
                if (rmc.enabled){
                    rovItens.rigMasterControls.Add(rmc);
                    rmc.enabled = false;
                }
            }

            foreach (T4FORCE2 t4f2 in rov.GetComponentsInChildren<T4FORCE2>()) {
                if (t4f2.enabled){
                    rovItens.t4FORCE2s.Add(t4f2);
                    t4f2.enabled = false;
                }
            }

            foreach (RigControlNew rcn in rov.GetComponentsInChildren<RigControlNew>()) {
                if (rcn.enabled){
                    rovItens.rigControlNews.Add(rcn);
                    rcn.enabled = false;
                }
            }

            foreach (GenericTranslate gt in rov.GetComponentsInChildren<GenericTranslate>()) {
                if (gt.enabled){
                    rovItens.genericTranslates.Add(gt);
                    gt.enabled = false;
                }
            }
        ...
        private void ReturnPhysics() 
        {
            if (preparedToReplay)
            {
                ...
                foreach (RigmasterTotal rmt in rovItens.rigMasterTotals)
                {
                    rmt.enabled = true;
                }
                foreach (RigMasterControl rmc in rovItens.rigMasterControls)
                {
                    rmc.enabled = true;
                }
                foreach (T4FORCE2 t4f2 in rovItens.t4FORCE2s)
                {
                    t4f2.enabled = true;
                }
                foreach (RigControlNew rcn in rovItens.rigControlNews)
                {
                    rcn.enabled = true;
                }
                foreach (GenericTranslate gt in rovItens.genericTranslates)
                {
                    gt.enabled = true;
                }
