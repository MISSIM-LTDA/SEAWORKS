    ...
    public void CreateSetup()
    {
        ...
        foreach(Transform child in GetComponentsInChildren<Transform>()) 
        {
            if (child.GetComponent<ObiActor>() != null)
            {
                ...
                GameObject startOfRope = null;
                GameObject endOfRope = null;

                foreach (ObiParticleAttachment attach in child.GetComponents<ObiParticleAttachment>())
                {
                    if (attach.particleGroup == groups[0]) {
                        if(attach.target.gameObject.name == "Cont (1)") { startOfRope = attach.target.transform.parent.gameObject; }
                        else { startOfRope = attach.target.gameObject; }
                        if (startOfRope.GetComponent<RecordedObject>() == null) { startOfRope.AddComponent<RecordedObject>(); }
                    }
                    else if (attach.particleGroup == groups[groups.Count - 1]) {
                        if (attach.target.gameObject.name == "Cont (1)") { endOfRope = attach.target.transform.parent.gameObject; }
                        else { endOfRope = attach.target.gameObject; }
                        if (endOfRope.GetComponent<RecordedObject>() == null) { endOfRope.AddComponent<RecordedObject>(); }
                    }
                }
