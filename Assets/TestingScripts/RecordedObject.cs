...
public class RecordedObject : MonoBehaviour
{
    ...
    protected IEnumerator SaveNewPathCoroutine(string path)
    {
        ...
        yield return StartCoroutine(GetObjectPosition());
    ...
    protected virtual IEnumerator GetObjectPosition()
    {
        if (transform.tag == "EFL_Parent")
        {
            ...
            record.RecordObjectStore.Add(new ObjectTransformToRecord(eflConnector1.localPosition, eflConnector1.localRotation));
            record.RecordObjectStore.Add(new ObjectTransformToRecord(eflConnector2.localPosition, eflConnector2.localRotation));
        }

        else{
            record.RecordObjectStore.Add(new ObjectTransformToRecord(transform.position, transform.rotation));
        }
    ...
    protected virtual void LoadPositions()
    {
        if (transform.tag == "EFL_Parent")
        {
            GameObject eflConnector1 = transform.GetChild(0).gameObject;
            GameObject eflConnector2 = transform.GetChild(1).gameObject;

            eflConnector1.GetComponent<Rigidbody>().isKinematic = true;
            eflConnector2.GetComponent<Rigidbody>().isKinematic = true;

            float e1ConnectedMassScale = eflConnector1.GetComponent<FixedJoint>().connectedMassScale;

            Destroy(eflConnector1.GetComponent<FixedJoint>());

            eflConnector1.transform.localPosition = record.RecordObjectStore[0].Postion;
            eflConnector1.transform.localRotation = record.RecordObjectStore[0].Rotation;
            eflConnector2.transform.localPosition = record.RecordObjectStore[1].Postion;
            eflConnector2.transform.localRotation = record.RecordObjectStore[1].Rotation;

            FixedJoint eflJoint = eflConnector1.AddComponent<FixedJoint>();
            eflJoint.connectedMassScale = e1ConnectedMassScale;
        ...
        {
            transform.position = record.RecordObjectStore[0].Postion;
            transform.rotation = record.RecordObjectStore[0].Rotation;
        }
