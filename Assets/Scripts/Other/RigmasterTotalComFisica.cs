...
[ExecuteInEditMode]
public class RigmasterTotalComFisica : MonoBehaviour
{
    //------------------------------------------------------------------------------------------------------------------------
    ...
    public GameObject RightClawTarget;
    public GameObject LeftClawTarget;

    private Rigidbody RightClaw_rig;
    private Rigidbody LeftClaw_rig;

    public KeyCode OpenClaw;
    public KeyCode CloseClaw;

    public float Claw_speed_rot;
    public ForceMode Claw_ForceMode;
    Vector3 m_EulerAngleVelocity_Claw;
    ...
    void FetchRigidBodies()
    {
        ...
        RightClaw_rig = RightClaw.GetComponent<Rigidbody>();
        LeftClaw_rig = LeftClaw.GetComponent<Rigidbody>();
    }
    void Set_Values()
    {
        ...
        m_EulerAngleVelocity_Claw = new Vector3(0, Claw_speed_rot, 0);
        Claw_canOpen = true;
    }
    void Start()
    {
    ...
    void ClawMovement()
    {
        ...
        if (Wrist_isRotating || Boom_isExtending || Arm_isMoving)
        {
            Jointify(RightClaw, Wrist_rig);
            Jointify(LeftClaw, Wrist_rig);
            return;
        }
        ...
        Destroy(RightClaw.GetComponent<FixedJoint>());
        Destroy(LeftClaw.GetComponent<FixedJoint>());

        Vector3 Force = new Vector3(-TargetX, 0, TargetZ) * dir * Time.fixedDeltaTime;
        RightClawTarget.transform.localPosition += Force;

        Force = new Vector3(TargetZ, 0, TargetX) * dir * Time.fixedDeltaTime;
        LeftClawTarget.transform.localPosition += Force;

        //Vector3 Torque = m_EulerAngleVelocity_Claw * dir * Time.fixedDeltaTime;
        
        //RightClaw_rig.AddRelativeTorque(Torque, Claw_ForceMode);
        //LeftClaw_rig.AddRelativeTorque(-Torque, Claw_ForceMode);
    }
